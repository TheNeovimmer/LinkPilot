...
function startReminderWorker(): void {
  cron.schedule('* * * * *', async () => {
    try {
      const now = new Date();
      const due = await reminders.dueNowAll(now);
      for (const reminder of due) {
        ...
        await auditService.log(reminder.userId, 'reminder.due', 'reminder', reminder.id, undefined, undefined, reminder.orgId);
      }
    } catch (err) {
  ...
  cron.schedule('* * * * *', async () => {
    try {
      ...
      const interviews = await prisma.interview.findMany({
        ...
        select: { id: true, userId: true, orgId: true, title: true, job: { select: { company: { select: { name: true } } } } },
      });
      for (const interview of interviews) {
        ...
        await auditService.log(interview.userId, 'interview.reminder', 'interview', interview.id, undefined, undefined, interview.orgId);
      }
    } catch (err) {
